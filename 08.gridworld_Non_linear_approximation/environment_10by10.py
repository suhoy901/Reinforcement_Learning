...
WIDTH = 10  # 그리드 가로
TRIANGLE_REWARD = -1
CIRCLE_REWARD = 100 # 크게 100으로 설정함
TIMESTEP_REWARD = 0
...
class Env(tk.Tk):
    def __init__(self):
        ...
        # 장애물 설정
        self.reset_reward()
        self.state_size = np.array(self.get_state()).shape
    ...
    def reset_reward(self):
        ...
        for i in range(1, HEIGHT - 1):
            first = np.random.choice(WIDTH // 2, 1, replace=False)
            self.set_reward([first, i], reward=TRIANGLE_REWARD)
            self.set_reward([first + 5, i], reward=TRIANGLE_REWARD)

        # #goal
        self.set_reward([HEIGHT - 1, WIDTH - 1], reward=CIRCLE_REWARD)
