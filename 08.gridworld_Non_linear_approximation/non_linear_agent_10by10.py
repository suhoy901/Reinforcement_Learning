...
class NonLinearAgent:
    def __init__(self, state_size, action_size):
        ...
        if self.load_model:
            self.epsilon = 0.01
            self.model.load_weights('./save_model/non_linear_10by10.h5')
        ...
        while True:
            ...
            if done:
                ...
                if e % 10 == 0:
                    ...
                    pylab.savefig("./save_graph/non_linear_10by10.png")

                if np.mean(scores[-min(10, len(scores)):]) > 99 \
                        or agent.epsilon < agent.epsilon_min:
                    agent.model.save_weights("./save_model/non_linear_10by10.h5")
                    agent.render = True
                else:
                    agent.render = False
