...
class NonLinearAgent:
    def __init__(self, state_size, action_size):
        ...
        if self.load_model:
            self.epsilon = 0.01
            self.model.load_weights('./save_model/non_linear_5by5.h5')
...
if __name__ == "__main__":
    ...
    agent = NonLinearAgent(state_size, action_size)
    ...
    for e in range(EPISODES):
        ...
        state = np.reshape(state, [1, state_size]) # state_size self.action_size
        ...
        while True:
            ...
            if done:
                ...
                if e % 50 == 0:
                    ...
                    pylab.savefig("./save_graph/non_linear.png")

                if np.mean(scores[-min(10, len(scores)):]) > 4 \
                        or agent.epsilon < agent.epsilon_min:
                    agent.model.save_weights("./save_model/non_linear.h5")
                    agent.render = True
                else:
                    agent.render = False
