...
class DQNAgent:
    def __init__(self, state_size, action_size):
        ...
        if self.load_model:
            self.model.load_weights("./save_model/cartpole_dqn_per_trained.h5")
    ...
    for e in range(EPISODES):
        ...
        while not done:
            ...
            if done:
                ...
                plt.savefig("./save_graph/cartpole_dqn_per.png")
                print("episode:", e, "  score:", score, "  memory length:",
                      step if step <= 2000 else 2000, "  epsilon:", agent.epsilon)

                # 이전 10개 에피소드의 점수 평균이 490보다 크면 학습 중단
                if np.mean(scores[-min(10, len(scores)):]) > 490:
                    agent.model.save_weights("./save_model/cartpole_dqn_per.h5")
                    sys.exit()
